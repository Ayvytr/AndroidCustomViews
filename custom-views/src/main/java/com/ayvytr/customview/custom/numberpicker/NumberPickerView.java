package com.ayvytr.customview.custom.numberpicker;
...
public class NumberPickerView extends View {
    ...
    // 是否关闭 mWrapSelectorWheelCheck 功能
    private boolean mDisableWrapSelectorWheelCheck;
    ...
    public boolean isDisableWrapSelectorWheelCheck() {
        return mDisableWrapSelectorWheelCheck;
    }

    /**
     * 设置禁用 {@link #mWrapSelectorWheelCheck} 功能，主要是在实际显示的条目小于 {@link #mShowCount} 时，是不是禁用掉数据
     * 循环滚动功能
     *
     * @param mDisableWrapSelectorWheelCheck {@code true} 禁用
     */
    public void setDisableWrapSelectorWheelCheck(boolean mDisableWrapSelectorWheelCheck) {
        this.mDisableWrapSelectorWheelCheck = mDisableWrapSelectorWheelCheck;
    }
    ...
    private void calculateFirstItemParameterByGlobalY() {
        ...
        if(mOnValueChangeListenerInScrolling != null) {
            ...
            if(mInScrollingPickedOldValue != mInScrollingPickedNewValue) {
                respondPickedValueChangedInScrolling(mInScrollingPickedOldValue + mMinValue,
    ...
    private void updateWrapStateByContent() {
        if(mDisableWrapSelectorWheelCheck) {
